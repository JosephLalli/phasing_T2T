...
## Set to true to recalculate phasing statistics
if [[ $chrom == 'PAR2' ]]; then 
...
then
    ...
    # phased_panel_vcf=$final_panel_dir/1KGP.CHM13v2.0.PAR1.snp_indel.phasing_qual_pass.phased.rare.bcf
    end_chrom=$PAR1_end
...
then
    ...
    grch38_region="chrX:155701383-156030895"
    whole_chrom=$region
    input_vcf=$initial_vcf_calls_folder/1KGP.CHM13v2.0.chrX.recalibrated.snp_indel.pass.mixed_ploidy.vcf.gz
    # phased_panel_vcf=$final_panel_dir/1KGP.CHM13v2.0.PAR2.snp_indel.phasing_qual_pass.phased.rare.bcf
    end_chrom=$PAR2_end
elif [[ $chrom == 'chrdebug' ]]
then
    chrom='debug'
    ...
    # phased_panel_vcf=$final_panel_dir/1KGP.CHM13v2.0.chr20.snp_indel.phasing_qual_pass.phased.rare.bcf
    end_chrom=12000000
...
then
    ...
    whole_chrom=$region
    grch38_region="chrX:2781479-155701383"
    input_vcf=$initial_vcf_calls_folder/1KGP.CHM13v2.0.chrX.recalibrated.snp_indel.pass.mixed_ploidy.vcf.gz
    # phased_panel_vcf=$final_panel_dir/1KGP.CHM13v2.0.chrX.snp_indel.phasing_qual_pass.phased.rare.bcf
    ...
    # phased_panel_vcf=$final_panel_dir/1KGP.CHM13v2.0.${chrom}.snp_indel.phasing_qual_pass.phased.rare.bcf
fi
...
pangenome_ground_truth_GRCh38=$basedir/${chrom}_working_GRCh38_evaluation/${chrom}_reference_pangenome.filtered_variants.biallelic.bcf
...
prior_run_source_folder=$basedir/${chrom}_working_clean_unmasked
...
then
    ...
    if [[ $chrom == 'chrX' ]] || [[ $chrom == 'PAR2' ]] || [[ $chrom == 'PAR1' ]]; then    # make missing/haploid into haploid
        bcftools view -r $region -Ov $pangenome_vcf | sed 's/\.|//g' \
        | bcftools norm --threads 2 -Ou -f $ref_fasta -m- - \
        ...
        | bcftools +fixploidy -Ou - -- -f 2 | bcftools +setGT -Ou - -- -t a -n p \
...
if [ ! -s $rare_variants_phased_ped_biallelic.csi ]; then
    ...
    | bcftools view  -c 1:minor --threads 8 -Ob -V other - > $rare_variants_phased_ped_biallelic \
...
if [ ! -s $phased_panel_vcf_3202_biallelic.tbi ]; then
    ...
    && bcftools view  --threads 8 $phased_panel_vcf_3202_biallelic ${phased_panel_vcf_3202_biallelic%%.vcf.gz}.bcf \
...
if [ ! -s $phased_panel_vcf_2504_biallelic.tbi ]; then
    ...
    bcftools view -Ou --threads 8 -S $unrelated_samples $rare_variants_phased_ped_biallelic \
    ...
    && bcftools view  --threads 8 $phased_panel_vcf_2504_biallelic ${phased_panel_vcf_2504_biallelic%%.vcf.gz}.bcf \
...
if [ ! -s $vcf_to_phase_no_parents.csi ]; then
    ...
    bcftools view -Ou --threads 8 -S $no_parents $vcf_to_phase > $vcf_to_phase_no_parents \
    | bcftools view -Ob -c 1:minor --threads 8 - \
...
if [ ! -s $phased_panel_vcf_2504.csi ]; then
    ...
    bcftools view -Ou --threads 8 -S $unrelated_samples $phased_panel_vcf_3202 > $phased_panel_vcf_2504 \
    | bcftools view -Oz --threads 8 -c 1:minor - \
...
if [ ! -s $vcf_phased_no_parents_rare_biallelic.csi ]; then
    bcftools concat --threads 8 -Ob -l $chrom_working_dir/*_tmp_noparents.rare.bcf > $vcf_phased_no_parents_rare_biallelic \
...
if [ ! -s $phased_panel_no_pangenome_biallelic.csi ]; then
    ...
    bcftools view -Ob --threads 8 -S ^$pangenome_and_parents --force-samples $phased_panel_vcf_2504_biallelic > $phased_panel_no_pangenome_biallelic \
...
if [[ ! -s $chrom_working_dir/phased_pangenome_noparents.biallelic.bcf ]]; then
    bcftools view --threads 8 -S $pangenome_samples --force-samples -Ou $vcf_phased_no_parents_rare_biallelic > $chrom_working_dir/phased_pangenome_noparents.biallelic.bcf \
...
if [[ ! -s $chrom_working_dir/${chrom}_3202_AC_AN.tsv ]]; then
    bcftools query -f "%ID\t%INFO/MAC\t%INFO/AN\n" $phased_panel_vcf_3202_biallelic > $chrom_working_dir/${chrom}_3202_AC_AN.tsv &
fi
if [[ ! -s $chrom_working_dir/${chrom}_2504_AC_AN.tsv ]]; then
    bcftools query -f '%ID\t%INFO/MAC\t%INFO/AN\n' ${phased_panel_vcf_2504_biallelic%%.vcf.gz}.bcf > $chrom_working_dir/${chrom}_2504_AC_AN.tsv  &
fi
if [[ ! -s $chrom_working_dir/${chrom}_2002_AC_AN.tsv ]]; then
    bcftools query -f "%ID\t%INFO/MAC\t%INFO/AN\n" $vcf_phased_no_parents_rare_biallelic > $chrom_working_dir/${chrom}_2002_AC_AN.tsv &
fi
if [[ ! -s $chrom_working_dir/${chrom}_2430_AC_AN.tsv ]]; then
    bcftools query -f "%ID\t%INFO/MAC\t%INFO/AN\n" $phased_panel_no_pangenome_biallelic > $chrom_working_dir/${chrom}_2430_AC_AN.tsv &
fi
...
if [[ "$recalc_phasing_stats" == 'true' ]]; then
    ...
    ## 4.2) Evaluate performance of pangenome samples phased with panel using trio concordance as ground truth
    ./SHAPEIT5_switch_static_v1.1.1 --validation $vcf_to_phase \
                                    --estimation $rare_variants_phased_pangenome_against_ref_biallelic \
                                    -R $whole_chrom --singleton \
                                    -P $pedigree \
...
if [[ ! -s $T2T_native_panel.csi ]] && [[ -s $phased_panel_vcf_2504_biallelic.tbi ]]; then
    ...
    && ./liftover_panel.sh $T2T_native_panel $GRCh38_lifted_panel /dev/shm/GRCh38_full_analysis_set_plus_decoy_hla.fasta /dev/shm/chm13v2-hg38.over.chain /mnt/data/lalli/nf_stage/genome_refs/t2t_liftover_workspace/grch38-chm13v2.sort.vcf.gz &
...
if [[ ! -s $GRCh38_native_panel.csi ]] && [[ -s ${GRCh38_native_panel%%.bcf}.vcf.gz.tbi ]]; then
    ...
    && ./liftover_panel.sh $GRCh38_native_panel $T2T_lifted_panel /dev/shm/chm13v2.0.fasta /dev/shm/hg38-chm13v2.over.chain /mnt/data/lalli/nf_stage/genome_refs/t2t_liftover_workspace/chm13v2-grch38.sort.vcf.gz &
...
if [ ! -s $SGDP_ground_truth_T2T.csi ]; then
    ...
    bcftools norm --threads 2 -Ou -r $region -f $ref_fasta -m- $SGDP_variants_T2T \
    | bcftools annotate -Ou --threads 8 -a $syntenic_site_location -c CHROM,FROM,TO --mark-sites +SYNTENIC \
                    -H '##INFO=<ID=SYNTENIC,Number=0,Type=Flag,Description="Syntenic with GRCh38 (source: https://s3-us-west-2.amazonaws.com/human-pangenomics/T2T/CHM13/assemblies/chain/v1_nflo/chm13v2-unique_to_hg38.bed)">' \
                    --set-id '%CHROM\_%POS\_%REF\_%FIRST_ALT' - \
...
if [ ! -s $SGDP_ground_truth_GRCh38.csi ]; then
    ...
    bcftools norm --threads 2 -Ou -r $grch38_region -f $GRCh38_fasta -m- $SGDP_variants_GRCh38 \
