...
if [[ "$test_run" == 'true' ]]; then
    chroms="15_test 22_test"
else
    chroms=$(seq 1 14) X $(seq 15 22) PAR1 PAR2
fi
...
do
    ...
    chrom_working_dir=$basedir/working_directories/${chrom}_working_${run_suffix}

    if [[ "$test_run" == 'true' ]]; then
        chrom=$(echo $chrom | cut -f 1 -d '_')
    fi

    phased_panel_location=$basedir/phased_panels/phased_${genome}_panel_${run_suffix}
    ...
    # if [[ $genome == 'GRCh38' ]]; then
    #     phased_panel_location=$basedir/phased_panels/phased_GRCh38_panel_${run_suffix}
    #     phased_panel_vcf_2504_biallelic=$phased_panel_location/1KGP.GRCh38.${chrom}.recalibrated.snp_indel.pass.phased.biallelic.2504.bcf
    # else
    #     phased_panel_vcf_2504_biallelic=$phased_panel_location/1KGP.CHM13v2.0.${chrom}.recalibrated.snp_indel.pass.phased.native_maps.biallelic.2504.bcf
    # fi
...
rm -f $basedir/working_directories/*working*_${run_suffix}/syntenic-nonsyntenic_vartype.*.tsv