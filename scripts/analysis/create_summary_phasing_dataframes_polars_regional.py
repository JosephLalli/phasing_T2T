...
if args.GRCh38_run_suffix:
    grch38_suffix = '_' + args.GRCh38_run_suffix
else:
    grch38_suffix = ''
...
for chrom in contigs:
    for genome, run_suffix in [('GRCh38', grch38_suffix),
                               ('CHM13v2.0', t2t_suffix)]:
        contig_variant_data = f"{summary_statistics_folder}/variant_frequency_stats/{genome}/1KGP.{genome}.{chrom}.snp_indel.phasing_qual_pass.fully_annotated.tsv.gz"
...
# print ('skipping variant stat writing')
                    ...
                    try:
                        ...
                        if len(file) == 1:  ##, f"multiple files found for {phasing_stats_folder}/{prefix}_{contig}_0.2.*{shapeit5_stat_suffixes[statistic]}*:\n{file}"
                            ...
                            if len(file) == 1:
                                ...
                                raise IndexError(f"no files found for {phasing_stats_folder}/{prefix}_{contig}_CHM13v2.0.*{shapeit5_stat_suffixes[statistic]}*:\n{file}")
                        else:
                            raise IndexError*(f"multiple files found for {phasing_stats_folder}/{prefix}_{contig}_CHM13v2.0.*{shapeit5_stat_suffixes[statistic]}*:\n{file}")
                    except IndexError:
                        raise FileNotFoundError(f"no files found for {phasing_stats_folder}/{prefix}_{contig}.{shapeit5_stat_suffixes[statistic]}*")
...
glimpse2_headers={'syntenic_maf_overall_bins': {'error.grp':  ('index','group_name','num_variants','mean_AF','num_AA_mismatches','num_AA','num_Aa_mismatches','num_Aa','num_aa_mismatches','num_aa','percent_AA_mismatches','percent_Aa_mismatches','percent_aa_mismatches'),
                  ...
                  'r2_bins':                   {'error.grp':  ('var_category_id','group_name','num_variants','mean_AF','num_AA','num_Aa','num_aa','num_calls_filtered_out','num_AA_matches','num_Aa_matches','num_aa_matches','num_AA_mismatches','num_Aa_mismatches','num_aa_mismatches','percent_AA_mismatches','percent_Aa_mismatches','percent_aa_mismatches','best_gt_rsquared','imputed_ds_rsquared'),
                                                'error.spl':  ('var_category_id','id','sample_name','num_AA','num_Aa','num_aa','num_calls_filtered_out','num_AA_matches','num_Aa_matches','num_aa_matches','num_AA_mismatches','num_Aa_mismatches','num_aa_mismatches','percent_AA_mismatches','percent_Aa_mismatches','percent_aa_mismatches','non_reference_discordance_percent','best_gt_rsquared','imputed_ds_rsquared'),
                                                'rsquare.grp':('group_name','num_variants','mean_AF','best_gt_rsquared','imputed_ds_rsquared')}}
if os.path.exists("expected_report_names.txt"):
    os.remove("expected_report_names.txt")
if os.path.exists("missing_report_names.txt"):
    os.remove("missing_report_names.txt")
for genome in ('T2T','GRCh38'):
    for variant_set in ('', '_snps', '_no_singletons', '_no_singletons_snps'): #, '_no_coinflips'):
    # for variant_set in ('', '_no_singletons'): #, '_no_coinflips'):
        # for ground_truth in ('SGDP',):
        for ground_truth in ('SGDP', 'pangenome'):
            for panel in ('native_panel','native_panel.common_variants','lifted_panel','lifted_panel.common_variants'):#, 'native_panel.GWAS_filtered.info_cutoff_0.3', 'lifted_panel.GWAS_filtered.info_cutoff_0.3', 'native_panel.GWAS_filtered.info_cutoff_0.7', 'lifted_panel.GWAS_filtered.info_cutoff_0.7', 'native_panel.GWAS_filtered.info_cutoff_0.9', 'lifted_panel.GWAS_filtered.info_cutoff_0.9'):
                for ancestry in ancestries:
                    # $outfolder/$genomic_variants.$dataset.$run
                    # $imputation_statistics_folder/{genome}{variant_set}.{ground_truth}.{panel}
                    # Unified naming: {genome}{variant_set}.{panel}.{ground_truth}[.modifiers][.ancestry].glimpse2_concordance
                    if ancestry == 'all':
                    ...
                    for bin_grouping in glimpse2_headers.keys():
                        ...
                        if (ancestry != 'all') and (ground_truth == 'pangenome'):
                            ...
                            continue
                        # if (ancestry != 'all') and (variant_set in ['_no_singletons', '_no_coinflips']):
                        #     # No singletons files don't have ancestry-specific versions
                        #     continue 
                            ...
                            try:
                                report = pd.read_csv(base_report_name + '_' + bin_grouping + '.' + report_name + '.txt.gz', sep=' ',
                                                     header=None, comment='#', names=glimpse2_headers[bin_grouping][report_name]
                                                              ...
                                                              panel=panel,
                                                              file=base_report_name + '_' + bin_grouping + '.' + report_name + '.txt.gz')
                            except Exception:
                                if bin_grouping != 'r2_bins':
                                    print (f'Could not find {base_report_name + "_" + bin_grouping + "." + report_name + ".txt.gz"}')
                                continue
                        ...
                        try:
                            ...
                            if 'info' in base_report_name + '_' + bin_grouping + '.rsquare.grp.txt.gz':
                                continue
                            else:
